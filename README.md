...
Absorbance is dimensionless; these wavelength-derivative units follow from using nm. **Normalized and ratio curves are transformed signals, not raw absorbance**, so their ordinate labels may be relative or mixed-signal labels instead. The initial **D0** X display limits are **200–800 nm**, irrespective of the imported spectrum's extent. For nonnegative absorbance spectra, the default Y minimum is **0**, and the automatic Y maximum rounds upward in **0.5-absorbance increments** according to the largest visible reading (for example, a maximum of 0.79 gives an upper limit of 1.0). Negative data are not suppressed by this automatic rule. Explicit user-selected Y limits take precedence. **Fit to data** adjusts the display range without changing the underlying observations. D0 Y-axis tick labels use two fixed decimal places, and X-axis tick labels use one fixed decimal place. D1–D4 Y tick labels adapt their precision to the derivative scale. The optional **Publication layout for derivatives D1–D4** fits the X-axis to the complete range of available derivative readings (without threshold-based truncation), rounds automatic Y limits outward to readable intervals, writes the inverse power-of-ten scaling explicitly in the Y-axis title (for example, `×10⁵ d⁴A/dλ⁴ (nm⁻⁴)` when the plotted tick values equal the derivative multiplied by 10⁵), applies consistent tick decimals including zero, uses 50-nm X ticks for wavelength spans up to 400 nm in automatic tick mode, renders the scale factor, derivative order and wavelength-unit exponent with explicit SVG superscripts; places a lightly framed white legend in the unobstructed top-right plot region when feasible (otherwise keeps the external legend), and uses stronger line weights and larger axis fonts. The on-page duplicate legend is suppressed for D1–D4 in publication mode. It leaves D0 formatting unchanged and restores the prior axis fields when turned off. Manually specified axis limits remain available; do not use automatic cropping when a larger wavelength context is scientifically required. Interactive cursor/axis hit targets are excluded from SVG and PNG exports.
